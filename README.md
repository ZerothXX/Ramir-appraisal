...
PyCharm 中右键运行 `app.py`（或命令行 `python app.py`），浏览器打开 http://127.0.0.1:8000/ 即可使用：

1. **封面页**：以 fairy.png 提取的眼睛为主视觉，蓝→浅蓝放射渐变背景，点"进入鉴定 →"进入选图页；
2. **选图页**：点击或拖拽本地图片（前端自动压缩后上传），点"开始鉴定"跳转思考页；
3. **思考页**：眼睛图标呼吸 + 涟漪动画，同时调用后端 `POST /api/predict` 推理（复用 `test.py` 的 `predict_image` 管线，模型全局单例只加载一次，首次请求会自动后台预热）；
4. **结果页**：展示原图与鉴定结果（结论 / 分析分行渲染）。
...
<img src="outputs/curve/loss_per_epoch.png" width="600" height="300">
