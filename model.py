...
* 所有超参数一律来自 config.py（函数签名中的 config 参数即调用方传入的
  config 模块本身）。
...
# config.LORA_TARGET_MODULES 为空时的兜底扫描关键字（规格 6.2）
...
def load_base_model_and_processor(config):
    ...
    * attn_implementation 用 "sdpa"（torch 2.2.1 内置），不用 flash_attention_2。
    """
    logger.info("Loading base model and processor from %s ...", config.BASE_MODEL_NAME_OR_PATH)
    ...
    logger.info("Base model loaded, device=%s", config.DEVICE)
...
def apply_lora(model, config):
    ...
    若 config.LORA_TARGET_MODULES 为空，自动扫描 nn.Linear 并按常见注意力投影
    关键字兜底，最终实际使用的 target_modules 会打印出来供核对。
    """
    if peft is None:
        raise ImportError("peft 未安装，请先执行: pip install peft（训练/推理测试阶段才需要）")

    keywords = config.LORA_TARGET_MODULES
    if not keywords:
        print("[apply_lora] config.LORA_TARGET_MODULES 为空，使用兜底关键字: "
    ...
    if not target_modules:
        raise RuntimeError(
            "未扫描到任何可 LoRA 的 nn.Linear 模块，请检查 config.LORA_TARGET_MODULES 关键字: "
            + str(keywords)
        )
...
def load_lora_checkpoint(base_model, adapter_dir):
    ...
    if not os.path.isfile(os.path.join(adapter_dir, "adapter_config.json")):
        raise FileNotFoundError(
            f"LoRA adapter 目录不存在或缺少 adapter_config.json: {adapter_dir}"
        )
...
def build_inputs(processor, images, texts):
    ...
    流程：texts 先按 config.MAX_TEXT_LENGTH 做字符串级截断 -> apply_chat_template
    ...
    """
    truncated = [_truncate_text_to_max_len(processor.tokenizer, t, config.MAX_TEXT_LENGTH)
                 for t in texts]
    messages = [
...
def build_train_batch(processor, images, prompts, targets):
    ...
    * 已用 DL 解释器实测（256x256 图，grid 18x18，merge_size=2，N=81）：
    ...
    # 推理侧的 build_inputs 才按 config.MAX_TEXT_LENGTH 截断。
