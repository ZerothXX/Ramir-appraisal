...
* 所有路径与超参数一律从 config1.py 读取，本文件不硬编码任何路径或数值；
...
def scan_dataset(dataset_dir, desc_dir, closed_set_dirs):
    ...
    参数：
        dataset_dir:   数据集根目录（如 config1.DATASET_DIR）
        desc_dir:      描述 json 目录（如 config1.DATASET_DESC_DIR）
        closed_set_dirs: 闭集文件夹名单（如 config1.CLOSED_SET_DIRS），不在名单内的
...
class AniTypeDataset(Dataset):
    ...
    目标文本格式（规格 3.1，label 已含"型拉米尔"后缀，直接拼接不重复追加）：
        config1.TRAIN_WITH_ANALYSIS=True  -> "结论：{label_text}\n分析：{analysis_text}"
        config1.TRAIN_WITH_ANALYSIS=False -> "结论：{label_text}"
    ...
    def __init__(self, samples, processor):
        ...
        self.with_analysis = config1.TRAIN_WITH_ANALYSIS
        self._images = []
        self._texts = []
        skipped = 0
