...
所有超参数一律来自 config1.py，本文件不硬编码任何数值/路径。
...
import config1
from data1 import AniTypeDataset, make_collate_fn, scan_dataset, train_val_split
from model1 import apply_lora, load_base_model_and_processor, save_lora_checkpoint
from utils1 import ensure_dir, get_logger, plot_curve, save_metrics_json, set_seed

logger = get_logger("train")


def move_to_device(batch):
    """把 batch 中的 tensor 搬到 config1.DEVICE（非 tensor 字段原样透传）。"""
    return {k: (v.to(config1.DEVICE) if isinstance(v, torch.Tensor) else v)
...
def main():
    set_seed(config1.RANDOM_SEED)
    ensure_dir(config1.OUTPUT_TRAIN_MODELS_DIR)
    ensure_dir(config1.OUTPUT_CURVE_DIR)
    ...
    logger.info("训练超参: epochs=%d batch=%d accum=%d lr=%g wd=%g warmup=%g sched=%s mp=%s "
                ...
                config1.NUM_EPOCHS, config1.BATCH_SIZE, config1.GRADIENT_ACCUMULATION_STEPS,
                config1.LEARNING_RATE, config1.WEIGHT_DECAY, config1.WARMUP_RATIO,
                config1.LR_SCHEDULER_TYPE, config1.MIXED_PRECISION, config1.LORA_R,
                config1.LORA_ALPHA, config1.LORA_DROPOUT, config1.GRADIENT_CHECKPOINTING)
    ...
    # ---- 数据 ----
    samples = scan_dataset(config1.DATASET_DIR, config1.DATASET_DESC_DIR, config1.CLOSED_SET_DIRS)
    train_samples, val_samples = train_val_split(
        samples, config1.TRAIN_VAL_SPLIT_RATIO, config1.RANDOM_SEED)
    ...
    train_loader = DataLoader(
        ...
        batch_size=config1.BATCH_SIZE, shuffle=True, num_workers=0, collate_fn=collate_fn)
    val_loader = DataLoader(
        AniTypeDataset(val_samples, processor),
        batch_size=config1.BATCH_SIZE, shuffle=False, num_workers=0, collate_fn=collate_fn)
    ...
    optimizer = torch.optim.AdamW(
        trainable_params, lr=config1.LEARNING_RATE, weight_decay=config1.WEIGHT_DECAY)

    micro_steps_per_epoch = len(train_loader)
    steps_per_epoch = math.ceil(micro_steps_per_epoch / config1.GRADIENT_ACCUMULATION_STEPS)
    ...
    total_steps = max(
        micro_steps_per_epoch * config1.NUM_EPOCHS // config1.GRADIENT_ACCUMULATION_STEPS, 1)
    warmup_steps = int(total_steps * config1.WARMUP_RATIO)
    scheduler = get_scheduler(
        name=config1.LR_SCHEDULER_TYPE,
        optimizer=optimizer,
    ...
    if config1.MIXED_PRECISION == "bf16":
        amp_dtype, scaler = torch.bfloat16, None
    elif config1.MIXED_PRECISION == "fp16":
        amp_dtype, scaler = torch.float16, torch.cuda.amp.GradScaler()
    elif config1.MIXED_PRECISION == "no":
        amp_dtype, scaler = None, None
    else:
        raise ValueError(f"config1.MIXED_PRECISION 取值非法: {config1.MIXED_PRECISION}"
                         f"（应为 'no' / 'fp16' / 'bf16'）")
    ...
    # ---- 训练循环 ----
    for epoch in range(1, config1.NUM_EPOCHS + 1):
        epoch_loss_sum = 0.0   # epoch 级真实 loss 之和
        epoch_micro_steps = 0
        window_loss_sum = 0.0  # 当前 optimizer step 窗口内 display loss 之和
        window_micro_steps = 0

        pbar = tqdm(train_loader, desc=f"Epoch {epoch}/{config1.NUM_EPOCHS}", unit="batch")
        for batch in pbar:
            batch = move_to_device(batch)
            with autocast_ctx:
                outputs = model(**batch)
            # 除以累积步数，使多步累积的梯度等于平均梯度
            loss = outputs.loss / config1.GRADIENT_ACCUMULATION_STEPS
            if scaler is not None:
            ...
            epoch_loss_sum += display_loss * config1.GRADIENT_ACCUMULATION_STEPS
            epoch_micro_steps += 1
            window_loss_sum += display_loss
            window_micro_steps += 1
            acc_counter += 1

            if acc_counter % config1.GRADIENT_ACCUMULATION_STEPS == 0:
                global_step += 1
                grad_norm = run_optimizer_step(optimizer, scaler, trainable_params,
                                               config1.MAX_GRAD_NORM, scheduler)
                ...
                if global_step % config1.LOG_EVERY_N_STEPS == 0:
                ...
                if global_step % config1.SAVE_STEPS == 0:
                    save_lora_checkpoint(
                        model, os.path.join(
                            config1.OUTPUT_TRAIN_MODELS_DIR,
                            f"epoch{epoch}_step{global_step}"))
                    save_lora_checkpoint(
                        model, os.path.join(config1.OUTPUT_TRAIN_MODELS_DIR, "latest"))

                # step 级验证（可选，做了就记录到 metrics）
                if global_step % config1.EVAL_STEPS == 0:
                    ...
                    best_val_loss = maybe_save_best(
                        ...
                        os.path.join(config1.OUTPUT_TRAIN_MODELS_DIR, "best"))
                    gc.collect()
            ...
            pbar.set_postfix(loss=f"{display_loss * config1.GRADIENT_ACCUMULATION_STEPS:.4f}")
            del batch
        ...
        if config1.EARLY_STOP_PATIENCE > 0 and val_loss is not None:
            if val_loss < best_val_loss:
                epochs_without_improvement = 0
            else:
                epochs_without_improvement += 1
                if epochs_without_improvement >= config1.EARLY_STOP_PATIENCE:
                    logger.info("早停触发：val loss 连续 %d 个 epoch 未下降"
                                "（当前 %.4f，历史最优 %.4f），提前结束训练",
                                config1.EARLY_STOP_PATIENCE, val_loss, best_val_loss)
        ...
        save_lora_checkpoint(
            model, os.path.join(config1.OUTPUT_TRAIN_MODELS_DIR, f"epoch{epoch}_step{global_step}"))
        save_lora_checkpoint(model, os.path.join(config1.OUTPUT_TRAIN_MODELS_DIR, "latest"))
    ...
    # ---- 落盘：原始数据 + 曲线图 ----
    metrics["early_stop_patience"] = config1.EARLY_STOP_PATIENCE
    metrics["early_stop_triggered"] = early_stop_triggered
    save_metrics_json(metrics, os.path.join(config1.OUTPUT_CURVE_DIR, "metrics.json"))
    plot_curve(
        metrics["step"], {"train_loss": metrics["loss"]},
        "Training loss per optimizer step", "Step", "Loss",
        os.path.join(config1.OUTPUT_CURVE_DIR, "loss_per_step.png"))
    plot_curve(
        metrics["epoch"], {"train_loss": metrics["epoch_train_loss"],
                           "val_loss": metrics["epoch_val_loss"]},
        "Train/Val loss per epoch", "Epoch", "Loss",
        os.path.join(config1.OUTPUT_CURVE_DIR, "loss_per_epoch.png"))
    plot_curve(
        metrics["step"], {"lr": metrics["lr"]},
        "Learning rate per optimizer step", "Step", "LR",
        os.path.join(config1.OUTPUT_CURVE_DIR, "learning_rate.png"))
    logger.info("指标原始数据与曲线已保存到 %s", config1.OUTPUT_CURVE_DIR)
